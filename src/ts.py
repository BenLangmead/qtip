...
def go(args, aligner_args, aligner_unpaired_args, aligner_paired_args):
    ...
    def _get_pass3_file_prefix():
        ...
        if args['write_all'] or args['write_tandem_intermediates']:
            def _purge():
                pass
            return join(odir, 'rewrite_intermediates'), _purge
        else:
    ...
    cmd = "%s ifs -- %s -- %s -- %s -- %s" % \
          (parse_input_exe, _get_passthrough_args(parse_input_exe), sam_fn, ' '.join(args['ref']), pass1_prefix)
    ...
    cmd = "%s f -- %s -- %s -- %s -- %s" % \
          (parse_input_exe, _get_passthrough_args(parse_input_exe), sam_fn, ' '.join(args['ref']), pass2_prefix)
    logging.info('  running "%s"' % cmd)
    ret = os.system(cmd)
    if ret != 0:
        raise RuntimeError("qsim-parse returned %d" % ret)
    ...
    cmd = "%s %s -- %s -- %s -- %s" % \
          (rewrite_exe, _get_passthrough_args(rewrite_exe), sam_fn, predictions_fn, pass3_prefix)
    logging.info('  running "%s"' % cmd)
    ret = os.system(cmd)
    if ret != 0:
