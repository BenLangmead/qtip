...
struct Alignment {
    ...
    Alignment() {
        clear();
        rf_aln_buf = (char *)malloc(BUFSZ);
        rf_aln_buf_len = BUFSZ;
        rd_aln_buf = (char *)malloc(BUFSZ);
        rd_aln_buf_len = BUFSZ;
    }
 ...
 * No guarantee about state of strtok upon return.
 */
static void print_unpaired(Alignment& al, int ordlen, FILE *fh_model, FILE *fh_recs) {
